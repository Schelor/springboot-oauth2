...
##  启动顺序与测试
```text
  # 1. 先启动 AuthServer (8080)
  cd springboot-authserver && ./gradlew bootRun

  # 2. 再启动 Resource Server (8081)
  cd springboot-oauth2-server && ./gradlew bootRun

  # 3. 再启动 Client (8082)
  cd springboot-oauth2-client && ./gradlew bootRun

  # 4. 测试完整链路
  curl "http://localhost:8082/call-hello?name=Alice&age=25"
  # 预期返回：{"name":"Alice","age":"25"}

  核心流程：Client 调用 8082/call-hello → 自动向 8080/auth/oauth2/token 申请 JWT → 带 Bearer Token 请求 8081/hello → Resource
   Server 本地用 JWKS 公钥验签（无需回调 AuthServer）→ 返回参数 JSON。
